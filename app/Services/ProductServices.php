...
class ProductServices
{
    ...
    public function addLoss($productAddLossRequest, Product $Product)
    {
        $no_of_damage = $productAddLossRequest->no_of_damage ?? 0;
        $no_of_lost = $productAddLossRequest->no_of_lost ?? 0;
        $no_of_lost_product = $no_of_damage + $no_of_lost;
        $no_of_product = $Product->no_of_product - $no_of_lost_product;
        $Product->update(['no_of_product' => $no_of_product]);
        return ['no_of_damage' => $no_of_damage, 'no_of_lost' => $no_of_lost, 'product_id' => $Product->id];
    }

    public function addStock($productAddRequest, Product $Product)
    {
        $no_of_stock = $productAddRequest->no_of_in_product;
        $no_of_product = $Product->no_of_product + $no_of_stock;
        Transction::create(['no_of_in_product' => $no_of_stock, 'product_id' => $Product->id]);
